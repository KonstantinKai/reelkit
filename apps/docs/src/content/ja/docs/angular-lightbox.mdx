...
<Callout type="info" title="オーバーレイが描画される場所">
  オーバーレイを `document.body` の末尾へ移す React と Vue の lightbox と違い、`rk-lightbox-overlay` は置いた場所に描画され、`position: fixed` で画面を覆います。`transform`、`perspective`、`filter`、`will-change: transform` を持つ祖先の中には置かないでください。その祖先がオーバーレイの包含ブロックになり、lightbox はビューポートではなくその祖先の大きさになり、祖先の `overflow` で切り取られます。独自のスタッキングコンテキストを作る祖先も、`--rk-lightbox-overlay-z` で lightbox をどこまで持ち上げられるかを制限します。その代わり、どの祖先に設定した `--rk-lightbox-*` トークンも lightbox に届きます。
</Callout>
...
組み込みのサウンドボタンは、`SoundStateService` が `<rk-lightbox-overlay>` の上で提供されているときだけ表示されます。スライドはあなたのテンプレートから描画されるので、その共有インスタンスだけが切り替えを動画まで届けます。ないときは `setLightboxVideoMuted` で要素を直接操作してください。
