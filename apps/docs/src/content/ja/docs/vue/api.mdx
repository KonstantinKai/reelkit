...
タグ: `<Observe :signals="[…]">` — いずれかのシグナルが変わったとき、既定のスロットだけを描き直し、周りには手を触れません。

レンダー関数やテンプレートで core のシグナルを読んでも、それだけでは購読になりません。ふつうは [`toVueRef`](#tovueref) を使いますが、これは呼び出したコンポーネントにシグナルを結び付けるため、よく変わるシグナルではそのコンポーネント全体が描き直されます。`Observe` はより狭い境界です。値を見せている部分だけを包めば、残りはそのまま動きません。ストーリープレイヤーがヘッダーとプログレスバーを更新しつつ、どちらのスライダーも描き直さずに済ませているのがこの仕組みです。

| Prop | 型 | 説明 |
| --- | --- | --- |
| `signals` | `readonly Subscribable[]` | 追いかけるシグナル。別の組を渡すと購読が張り替わり、同じ組ならそのまま保たれます。アンマウント時にはすべて解放されます。 |

プロパティの型は `ObserveProps` として公開されています。

<CodeBlock code={observe} language="vue" />

コンポーネント全体がシグナルを追うなら `toVueRef`、その一部だけなら `Observe` を選んでください。
