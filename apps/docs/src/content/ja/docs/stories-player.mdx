...
`desktopLayout="carousel"` を指定すると、プレーヤーが Instagram のウェブ版のようなレイアウトになります。アクティブなストーリーはいつもの大きさのまま中央に置かれ、その左右に隣のグループが最大 2 つずつ、小さく暗めのカードとして並びます。カードをクリックするとそのグループが開き、グループが切り替わる間カードが横へスライドします。リング一覧に渡すのと同じ `viewedState` のマップを渡すと、見終えたグループのカードは控えめなリングになります。
...
- **カードにはグループが開く位置が表示されます**：このセッションで離れたときのストーリー、なければ `resumeStoryIndex` が返すストーリーです。動画のポスターか画像そのものを表示します。どちらもないストーリー（グラデーション上のテキストのストーリーなど）は、`renderSlide` がプレーヤーの大きさで描き、それを縮小して表示します。ポスターのない動画ではカードの背景が見えます。
...
| `desktopLayout` | `'single' \| 'carousel'` | 'single' | デスクトップでのレイアウト。'carousel' ではアクティブなストーリーの横に隣のグループをプレビューカードで表示し、グループ間をスライドで切り替えます。スマートフォンでは常にストーリーだけを表示します。 |
| `viewedState` | `Map<string, number>` | - | 作成者 id ごとの視聴済みストーリー数（viewedCounts() から）。見終えたグループのカードには控えめなリングが表示されます。 |
