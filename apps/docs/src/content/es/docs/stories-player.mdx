...
- **Cada tarjeta muestra dónde se abre el grupo**: la historia en la que se dejó durante esta sesión o, si no, la que devuelve `resumeStoryIndex`. Enseña el póster del vídeo o la propia imagen. Una historia sin ninguno de los dos, como un texto sobre un degradado, la dibuja tu `renderSlide` al tamaño del reproductor y se muestra reducida; un vídeo sin póster muestra el fondo de la tarjeta.
...
| `desktopLayout` | `'single' \| 'carousel'` | 'single' | Diseño en escritorio. 'carousel' muestra los grupos vecinos como tarjetas de vista previa junto a la historia activa y desliza entre grupos. En el móvil la historia siempre se muestra sola. |
| `viewedState` | `Map<string, number>` | - | Historias vistas por id de autor, de viewedCounts(). Las tarjetas del carrusel muestran el anillo apagado de un grupo visto hasta el final. |
