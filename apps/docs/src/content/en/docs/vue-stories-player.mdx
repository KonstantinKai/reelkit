...
- **A player mounted only when it opens reads too late.** The overlay reads the store once mounted, while it is still closed. With `v-if="open"` there is no such moment, so call `useAttachViewedState(() => viewed)` in the component that owns the controller; it attaches once mounted and lets go on unmount. The options, and the lower-level pieces this is built from, are on the [Stories Core](/docs/stories-core#viewed-controller) page.
