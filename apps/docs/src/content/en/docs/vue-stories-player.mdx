...
| #header | `{ author, story, storyIndex, isPaused, isMuted, isVideo, onToggleSound, onTogglePause, onClose }` | Replaces the default header (author, pause, sound, close) |
...
| #progressBar | `{ totalStories, activeIndex, progress, group }` | Replaces the canvas progress bar. `activeIndex` and `progress` are core signals — bridge them with `toVueRef`. |
...
| --- | --- |
| `CanvasProgressBar` | The canvas segmented progress bar, drawn straight from the timer signals. Handles a sliding window for groups with many stories. |
...
Extend `StoryItem` with your own fields and type the groups with it. Annotate a slot scope to keep the extra fields visible inside the template:
...
| `.rk-stories-ui-layer` | Overlay | UI overlay container (header, progress) |
...
The overlay captures focus on open and returns it to the trigger on close. Tab and Shift+Tab cycle through focusable elements inside; focus that escapes gets pulled back. Implemented with `captureFocusForReturn` and `createFocusTrap` from `@reelkit/core`.
