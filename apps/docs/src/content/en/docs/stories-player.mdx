...
- **A player mounted only when it opens reads too late.** The player reads the store in an effect that runs while it is still closed. With `{open && <StoriesOverlay … />}` there is no such moment, so call `useAttachViewedState(viewed)` in the component that owns the controller; it attaches on mount and lets go on unmount. The options, and the lower-level pieces this is built from, are on the [Stories Core](/docs/stories-core#viewed-controller) page.
...
- **Custom renderers are called for every group on screen.** `renderProgressBar` and `renderHeader` get `groupIndex` and `isActive`; for a neighbouring group `isActive` is `false` and the progress signals hold still.
- **A custom header sits in the swipe area.** A tap on it moves between stories unless it lands on a `button`, a link, or an element with `role="button"`, so make every control in it one of those.
...
| `renderHeader` | `(props: HeaderRenderProps<T>) => ReactNode` | - | Custom header renderer. Receives author, story, pause/mute state. With `chromePlacement="group"` it sits in the swipe area; see [Progress Bar and Header per Group](/docs/stories-player#chrome-placement). |
...
Reusable building blocks exported for composition in custom render props. Each one's props type is exported beside it — `CanvasProgressBarProps`, `StoryHeaderProps`, `ImageStorySlideProps`, `VideoStorySlideProps`, `StoriesRingProps`, `StoriesRingListProps` and `HeartAnimationProps` — for a component that wraps one:

### CanvasProgressBar [#canvasprogressbar]

High-performance canvas-based segmented progress bar. Renders segments for each story and animates the active segment fill via `requestAnimationFrame`. Supports a sliding window for groups with many stories. Pass `live={false}` for a bar that only needs to show where a group stands: it draws when its signals change and after a resize, with no animation loop.
...
`SoundProvider` and `useSoundState` carry the sound state the player shares with its slides. A custom slide that plays its own media reads the mute toggle through `useSoundState`. Inside the player a provider is already in place; a `VideoStorySlide` or custom slide rendered anywhere else needs a `SoundProvider` above it, or `useSoundState` throws.
...
| `.rk-stories-ui-layer` | Overlay | UI overlay container (header, progress, navigation). Inside every group slide with `chromePlacement="group"` |
...
| `.rk-stories-header-btn--desktop` | StoryHeader | Pause button, shown only above 768px wide |
...
| `.rk-stories-card-frame` | Carousel | Scaled-down `renderSlide` preview of a story with no image or poster; inert |
...
| `--rk-stories-container-radius-mobile` | `0` | Corners of the story canvas up to 768px wide |
| `--rk-stories-ui-z` | `15` | z-index of the UI layer (header, progress bar) |
...
| `--rk-stories-error-z` | `5` | z-index of the error state |
...
| `--rk-stories-error-gap` | `12px` | Space between the error icon and its text |
...
| `--rk-stories-header-gap` | `8px` | Space between the avatar, name and time |
| `--rk-stories-header-transition` | `200ms` | Header fade when it is hidden |
...
| `--rk-stories-header-actions-gap` | `8px` | Space between the header action buttons |
...
| `--rk-stories-header-spinner-track` | `rgba(255, 255, 255, 0.3)` | Buffering spinner track color |
| `--rk-stories-header-spinner-fg` | `#fff` | Buffering spinner arc color |
| `--rk-stories-header-spinner-duration` | `0.8s` | One turn of the buffering spinner |
...
| `--rk-stories-ring-gradient` | `none` | Internal: the ring component writes its gradient here on every render, so an override has no effect |
...
The overlay captures focus on open and returns it to the trigger on close. Tab and Shift+Tab cycle through focusable elements inside; focus that escapes (click outside, programmatic focus) gets pulled back. Implemented with `captureFocusForReturn` and `createFocusTrap` from `@reelkit/core`, re-exported by `@reelkit/react`.
...
The overlay renders nothing while closed and portals into `document.body` only once it opens, which happens on the client. `StoriesUrlOverlay` reads the address bar in an effect after mount, so a server render never opens it. `StoriesRingList` renders every ring unwatched on the server: the viewed store is read only after mount, so the server markup and the first client render agree. See the [SSR guide](/docs/ssr) for the wider picture.
