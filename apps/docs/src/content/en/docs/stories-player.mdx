...
- **A player mounted only when it opens reads too late.** The player reads the store in an effect that runs while it is still closed. With `{open && <StoriesOverlay … />}` there is no such moment, so call `viewed.attach()` yourself in an effect that returns its dispose. The options, and the lower-level pieces this is built from, are on the [Stories Core](/docs/stories-core#viewed-controller) page.
