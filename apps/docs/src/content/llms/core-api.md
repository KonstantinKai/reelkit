...
| `createStableIdCodec`      | `(hash: boolean) => UrlCodec<string>`                                                                     | The stable-id **wire**, exported for composing — the param text is the item's `id`, raw or (with `hash`) base64url-obscured. The stable-id analog of `indexCodec`: pair it with a locator of your own instead of taking the whole `urlStableIdKey`.                                                                                                                                                                                                     |
