...
| `desktopLayout` | `'single' \| 'carousel'` | 'single' | Layout no desktop. 'carousel' mostra os grupos vizinhos como cards de prévia ao lado do story ativo e desliza entre grupos. No celular o story sempre aparece sozinho. |
| `viewedState` | `Map<string, number>` | - | Stories vistos por id de autor, vindo de viewedCounts(). Os cards do carrossel mostram o anel apagado para um grupo assistido até o fim. |
