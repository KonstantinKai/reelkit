...
const index = toAngularSignal(controller.state.index, destroyRef);`}
      ...
      <section className="mb-12">
        ...
        <p className="text-slate-600 dark:text-slate-400 mb-4">
          ...
          Запис торкається лише запиту, тож шлях, фрагмент і повторювані ключі
          на кшталт <code>?tag=a&amp;tag=b</code> лишаються недоторканими. Кожна
          зміна повідомляє, чи Router додав запис на тій самій сторінці, замінив
          його чи пройшов історією, тож галерея, відкрита з{' '}
          <code>routerLink</code>, закривається одним кроком назад. Викликайте
          його в контексті впровадження; підписка на роутер звільняється через{' '}
