...
const overlayUrlStateOptions = [
  {
    ...
    default: 'required',
    description:
      'Параметр запиту, що несе активний слайд, наприклад "photo". Читається під час першого рендеру й не змінюється весь час життя компонента — щоб змінити, перемонтуйте його (задайте key).',
  },
  {
    prop: 'adapter',
    type: 'UrlAdapter',
    default: 'History API',
    description:
      'Система навігації, через яку читати й писати. У застосунку з роутером передайте адаптер на його базі, щоб власне місцеположення роутера не застаріло. Читається під час першого рендеру й не змінюється весь час життя компонента — щоб змінити, перемонтуйте його.',
  },
  {
    prop: 'codec',
  ...
  {() => (
    ...
    />
        ...
        <p className="text-slate-600 dark:text-slate-400 mb-2">
          ...
          Запис торкається лише запиту, тож шлях і хеш лишаються недоторканими.
          Кожна зміна повідомляє, чи роутер додав запис на тій самій сторінці,
          замінив його чи пройшов історією, тож галерея, відкрита з{' '}
          <code>&lt;Link&gt;</code>, закривається одним кроком назад.
        </p>
        <p className="text-slate-600 dark:text-slate-400 mb-2">
