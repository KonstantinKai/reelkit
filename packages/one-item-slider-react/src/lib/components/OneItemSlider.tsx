...
export interface OneItemSliderProps {
  ...
  apiRef?:
    | React.MutableRefObject<OneItemSliderPublicApi | null>
    | ((api: OneItemSliderPublicApi) => void);
...
const defaultKeyExtractor: NonNullable<OneItemSliderProps['keyExtractor']> = (
  index
) => index.toString();
...
export const createDefaultKeyExtractorForLoop =
  (
    count: number,
    keyPrefix?: string
  ): NonNullable<OneItemSliderProps['keyExtractor']> =>
