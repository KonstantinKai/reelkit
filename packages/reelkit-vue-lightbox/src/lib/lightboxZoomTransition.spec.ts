...
describe('lightboxZoomTransition', () => {
  it('at rest: current slide is fully opaque at scale 1', () => {
    const current = lightboxZoomTransition(
      _kSize * -1,
      1,
      1,
      _kSize,
  ...
  it('at rest: neighbors are hidden', () => {
    expect(
      lightboxZoomTransition(_kSize * -1, 0, 1, _kSize, 'horizontal').opacity,
    ).toBe(0);
    expect(
      lightboxZoomTransition(_kSize * -1, 2, 1, _kSize, 'horizontal').opacity,
    ).toBe(0);
  });
  ...
  it('fully off-screen slides are hidden with zIndex 0', () => {
    const farAway = lightboxZoomTransition(
      _kSize * -5,
      1,
      1,
      _kSize,
  ...
  it('zIndex flips at midpoint', () => {
    const nearCurrent = lightboxZoomTransition(
      _kSize * -1.3,
      2,
      1,
      _kSize,
    ...
    const nearNext = lightboxZoomTransition(
      _kSize * -1.7,
      2,
      1,
      _kSize,
  ...
  it('returns opacity 0 when primarySize is 0', () => {
    expect(lightboxZoomTransition(0, 0, 0, 0, 'horizontal')).toEqual({
      opacity: 0,
    });
  });
});
