...
describe('lightboxFadeTransition', () => {
  it('at rest: current slide is fully opaque', () => {
    const current = lightboxFadeTransition(
      _kSize * -1,
      1,
      1,
      _kSize,
  ...
  it('at rest: neighbors are fully transparent', () => {
    expect(
      lightboxFadeTransition(_kSize * -1, 0, 1, _kSize, 'horizontal').opacity,
    ).toBe(0);
    expect(
      lightboxFadeTransition(_kSize * -1, 2, 1, _kSize, 'horizontal').opacity,
    ).toBe(0);
  });
  ...
  it('current slide gets higher zIndex', () => {
    ...
    const neighbor = lightboxFadeTransition(
      _kSize * -1.5,
      2,
      1,
      _kSize,
  ...
  it('returns opacity 0 when primarySize is 0', () => {
    expect(lightboxFadeTransition(0, 0, 0, 0, 'horizontal')).toEqual({
      opacity: 0,
    });
  });

  it('clamps opacity to 0 (never negative)', () => {
    const farAway = lightboxFadeTransition(
      _kSize * -5,
      1,
      1,
      _kSize,
