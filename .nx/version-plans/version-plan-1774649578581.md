...
BREAKING: `useFullscreen` hook removed — import from `@reelkit/react` instead
BREAKING: `renderSlide` accepts a single `SlideRenderProps` object instead of separate parameters
BREAKING: `LightboxControlsRenderProps` renamed to `ControlsRenderProps`, `currentIndex` renamed to `activeIndex`
BREAKING: `LightboxSlideRenderProps` renamed to `SlideRenderProps`
