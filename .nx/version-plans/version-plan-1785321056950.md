...
`createLruCache` takes an optional backing Map as its third parameter and runs its eviction over that Map, so a caller can keep iterating and serializing a Map it owns
